...
2. Run `curl -fsSL https://raw.githubusercontent.com/nextster/chromium-bridge/main/install.sh | sh -s -- --host-only --extension-id ITEM_ID --no-codex --no-open --no-wait`, replacing `ITEM_ID` with this Store item's ID.
...
8. Run `~/.chromium-bridge/bin/chromium-bridge status` and confirm `connected` and `consented` are true.
