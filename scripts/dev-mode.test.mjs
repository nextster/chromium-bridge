...
test("dev link is idempotent and unlink restores bundled runtime", async () => {
  const fixture = await makeFixture("0.6.9", "0.6.9");
  try {
...
test("dev status reports stale versions and invalid moved checkout", async () => {
  const fixture = await makeFixture("0.6.9", "0.6.8");
  try {
    let status = await developmentStatus(fixture.options);
    assert.match(status.mismatches.join("\n"), /installed plugin 0\.6\.8 differs from repo 0\.6\.9/);
...
test("dev status reads the Codex plugin cache under the nextster marketplace", {
  ...
  const cachedPlugin = path.join(codexHome, "plugins", "cache", "nextster", "chromium-bridge", "0.6.9");
  const codexPath = path.join(fixture.root, "codex");
  try {
    await mkdir(path.join(cachedPlugin, ".codex-plugin"), { recursive: true });
    await writeFile(path.join(cachedPlugin, ".codex-plugin", "plugin.json"), JSON.stringify({ version: "0.6.9" }));
    ...
    await writeFile(codexPath, `#!/bin/sh\nprintf '%s' '${JSON.stringify({
      installed: [{ pluginId: "chromium-bridge@nextster", version: "0.6.9", source: { path: fixture.root } }]
    })}'\n`, { mode: 0o700 });
