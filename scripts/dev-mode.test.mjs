...
test("dev link is idempotent and unlink restores bundled runtime", async () => {
  const fixture = await makeFixture("0.7.3", "0.7.3");
  try {
...
test("dev status reports stale versions and invalid moved checkout", async () => {
  const fixture = await makeFixture("0.7.3", "0.7.2");
  try {
    let status = await developmentStatus(fixture.options);
    assert.match(status.mismatches.join("\n"), /installed plugin 0\.7\.2 differs from repo 0\.7\.3/);
...
async function makeFixture(repoVersion, installedVersion) {
  ...
  await writeFile(path.join(projectDir, "native-host", "src", "mcp-server.mjs"), "export {};\n");
  ...
  await writeFile(path.join(projectDir, "native-host", "src", "runtime-bootstrap.mjs"), "export const fixture = 1;\n");
  await writeFile(path.join(stateDir, "runtime", "runtime-bootstrap.mjs"), "export const fixture = 1;\n", { mode: 0o600 });
  await writeFile(path.join(stateDir, "runtime", "host.mjs"), "export {};\n", { mode: 0o600 });
  await writeFile(path.join(stateDir, "runtime", "cli.mjs"), "export {};\n", { mode: 0o600 });
  await writeFile(path.join(stateDir, "runtime", "mcp-server.mjs"), "export {};\n", { mode: 0o600 });
  ...
  return {
    ...
    projectDir,
    stateDir,
    options: { projectDir, stateDir, installedPluginPath: pluginDir, codexPath: null, claudePath: null, skipInstall: true },
...
test("dev status reads the Codex plugin cache under the nextster marketplace", {
  ...
  const cachedPlugin = path.join(codexHome, "plugins", "cache", "nextster", "chromium-bridge", "0.7.3");
  const codexPath = path.join(fixture.root, "codex");
  try {
    await mkdir(path.join(cachedPlugin, ".codex-plugin"), { recursive: true });
    await writeFile(path.join(cachedPlugin, ".codex-plugin", "plugin.json"), JSON.stringify({ version: "0.7.3" }));
    ...
    await writeFile(codexPath, `#!/bin/sh\nprintf '%s' '${JSON.stringify({
      installed: [{ pluginId: "chromium-bridge@nextster", version: "0.7.3", source: { path: fixture.root } }]
    })}'\n`, { mode: 0o700 });
    await chmod(codexPath, 0o700);
    const { installedPluginPath, ...options } = fixture.options;
    const status = await developmentStatus({ ...options, codexPath, codexHome });
    assert.equal(status.mcp.installed, true);
    assert.equal(status.mcp.cachedPath, cachedPlugin);
    assert.equal(status.mcp.bootstrapReady, true);
  } finally {
    await fixture.cleanup();
  }
});
...
test("dev link refuses to reuse a runtime bootstrap from another release", async () => {
  ...
  try {
    ...
    assert.equal(status.nativeHost.bootstrapCurrent, false);
    assert.match(status.mismatches.join("\n"), /bootstrap differs from the checkout/);
    await assert.rejects(linkDevelopment(fixture.options), /missing or differs from the checkout/);
  } finally {
    await fixture.cleanup();
  }
});
